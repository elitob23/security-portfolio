...
FIRST 3 THINGS I CHECK:
    ...
    Target Identity & Endpoint: Which user account and endpoint triggered the alert? Is this a standard user, a domain admin, or a service account? Is this behavior typical for their role?

    Connection Context: Was the connection inbound or outbound? What process initiated the network connection (e.g., web browser, system process, PowerShell, cmd.exe)? Was it a single packet or an ongoing stream?

    IP Reputation Lookup: Query the external IP in threat intelligence platforms (e.g., VirusTotal, AbuseIPDB). What is it flagged for (C2 infrastructure, malware delivery, scanning, phishing)? Who is the hosting provider?
...
FIRST 3 THINGS I CHECK:
    ...
    IP & Geolocation Analysis: Identify the source IP and geographical location. Look for previous successful logins from this IP or ISP in the directory logs (e.g., Microsoft Entra ID).

    User Baseline Deviation: Compare this sign-in with the user's historical sign-in history. Is the user currently traveling, on vacation, or working remotely?

    Resource & Session Activity: Review what resources were accessed during the anomalous session (e.g., SharePoint sites, admin portals, bulk file downloads). Are there concurrent detections or changes on the account?
...
FIRST 3 THINGS I CHECK:
    ...
    Time Delta vs. Distance: Calculate the time elapsed between the two sign-ins and the geographical distance. Is it physically possible to travel between them?

    VPN & ISP Verification: Analyze the network providers. Is one of the locations a known corporate VPN hub, cloud proxy, or cellular network carrier that might mask geolocation?

    Session Behavior & Auditing: Check the activity performed by both sessions. Is one session doing normal work while the other is attempting administrative actions, bulk downloads, or lateral movement?
...
FIRST 3 THINGS I CHECK:
    ...
    Source Asset & Role: Identify the source host initiating the RPC request. Is it a dedicated IT management workstation, an automated server, or a standard employee workstation?

    Actor Privilege & Credentials: Review the user or service account executing the command. Is it a member of the domain admins, a standard user, or a known service account?

    Process Lineage: Inspect the specific process making the RPC call. Is it a trusted IT management agent (e.g., RMM, backup agent, system service) or an interactive shell (e.g., cmd.exe, powershell.exe)?
